<template>
    ...
    <PageHeader
        ...
        :subtitle="isEdit ? 'Update category details, imagery, and visibility.' : 'Add a new category to organize your catalog.'"
        :crumbs="crumbs"
    >
        <template #titleBadge>
        ...
        <template #actions>
            <Button variant="secondary" size="sm" tag="a" :href="route('admin.category')">Cancel</Button>
    ...
    <form @submit.prevent="submit" enctype="multipart/form-data" class="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <div class="xl:col-span-2 space-y-6">
            <Card mode="flat" title="Details" subtitle="Basic information shown on the storefront.">
                <div class="space-y-5">
                    <FormField label="Name" required :error="errors?.name">
                        <Input v-model="form.name" placeholder="e.g. Kitchenware" :error="!!errors?.name" @input="generateSlug" />
                    </FormField>

                    <FormField label="Slug" hint="URL-friendly identifier, auto-generated from the name." :error="errors?.slug">
                        <Input v-model="form.slug" placeholder="e.g. kitchenware" :error="!!errors?.slug" />
                    </FormField>

                    <FormField label="Description" :error="errors?.description">
                        <Textarea v-model="form.description" rows="5" placeholder="Optional description for shoppers…" :error="!!errors?.description" />
                    </FormField>
                </div>
            </Card>
        </div>
        ...
        <aside class="space-y-6 xl:col-span-1">
            <Card mode="flat" title="Visibility">
                <div class="space-y-4">
                    <Switch v-model="form.status">
                        <span class="font-medium text-gray-900 dark:text-white/95">Active</span>
                        <span class="block text-[12px] text-gray-500 dark:text-gray-400">Show on the storefront.</span>
                    </Switch>
...
export default {
    ...
    computed: {
        ...
        isEdit() { return !!this.category?.id; },
        crumbs() {
            return [
                { label: 'Dashboard', href: route('admin.dashboard') },
                { label: 'Categories', href: route('admin.category') },
                { label: this.isEdit ? 'Edit' : 'New' },
            ];
        },
    },
    mounted() {
        this.previewImage = this.category?.image ? `/storage/category/${this.category.image}` : null;
    },
    methods: {
        generateSlug() {
        ...
        async handleFileChange(e) {
            ...
            try {
                ...
                try {
                    ...
                    console.warn('Category image compression failed, using original:', err);
                }
