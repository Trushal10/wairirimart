<template>
    <Head :title="`Configure ${partner.name}`" />

    <SettingsShell
        :title="`Configure ${partner.name}`"
        subtitle="Credentials are encrypted at rest. Leave blank to keep the existing value."
        :crumbs="crumbs"
    >
        <template #titleBadge>
        ...
        <template #actions>
            <Button variant="secondary" size="sm" tag="a" :href="route('admin.couriers.index')">
                <template #leading><ArrowLeft :size="14" /></template>
                Back
            </Button>
            <Button v-if="!unsupported" variant="ghost" size="sm" :loading="testing" @click="test">
        ...
        <Alert v-if="unsupported" variant="warning" title="No API adapter" class="mb-6">
            No API adapter is registered for this partner code — it can still be enabled as an in-house carrier, but no external API calls will be made.
        </Alert>
        ...
        <form @submit.prevent="submit" class="grid grid-cols-1 gap-6 xl:grid-cols-3">
            <div class="space-y-6 xl:col-span-2">
                    ...
                    <div class="space-y-5">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <FormField label="Provider code" hint="Fixed — matches the adapter.">
                                <Input :model-value="partner.code" disabled class="font-mono" />
                            </FormField>
                        </div>
                        <FormField label="Description" hint="Optional — displayed to admins only.">
                            <Textarea v-model="form.description" maxlength="500" :rows="2" placeholder="Notes about this partner…" />
                        </FormField>
                        <FormField label="Custom API base URL" hint="Optional override — leave blank to use the adapter's default endpoint.">
                            <Input v-model="form.api_base_url" type="url" placeholder="https://api.provider.com" />
                        </FormField>
                    </div>
                </Card>

                <!-- Mode & priority -->
                <Card mode="flat" title="Mode & priority" subtitle="Test mode routes shipments to the provider's sandbox.">
                    <div class="space-y-5">
                        <FormField label="Environment">
                            ...
                            <p class="mt-2 text-[12px] text-gray-500 dark:text-gray-400">
                                <template v-if="form.mode === 'live'">
                                    <span class="text-success-700 dark:text-success-400 font-semibold">Live mode</span> — real shipments will be created.
                                </template>
                                <template v-else>
                                    <span class="text-warning-700 dark:text-warning-400 font-semibold">Test mode</span> — safe for development. Sandbox endpoint.
                                </template>
                            </p>
                        </FormField>

                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <FormField label="Priority" hint="Higher priority is preferred when auto-assigning.">
                            ...
                            <FormField label="Availability">
                                <div class="space-y-3 pt-1">
                                    <Switch v-model="form.is_active">
                                        ...
                                        <span class="block text-[12px] text-gray-500 dark:text-gray-400">Available for shipping.</span>
                                    </Switch>
                                    <Switch v-model="form.is_default">
                                        <span class="font-medium text-gray-900 dark:text-white/95">Default courier</span>
                                        <span class="block text-[12px] text-gray-500 dark:text-gray-400">Auto-assign new orders.</span>
                                    </Switch>
                                </div>
                            </FormField>
                        </div>
                        ...
                        >
                            <div v-if="meta.type === 'password'" class="space-y-1.5">
                                <PasswordInput
                                    ...
                                    :placeholder="partner.credentials_masked?.[field] || (meta.required ? 'Required' : 'Optional')"
                                    :error="!!errors?.[`credentials.${field}`]"
                                />
                                <button
                                    v-if="partner.credentials_masked?.[field] && !revealed[field]"
                                    type="button"
                ...
                <!-- Shipping defaults -->
                <Card mode="flat" title="Shipping defaults" subtitle="Fallback package dimensions when a product doesn't specify them.">
                    <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
                        <FormField label="Length (cm)">
                            <Input v-model.number="form.config.default_length" type="number" step="0.1" min="0" />
                        </FormField>
                        <FormField label="Breadth (cm)">
                            <Input v-model.number="form.config.default_breadth" type="number" step="0.1" min="0" />
                        </FormField>
                        <FormField label="Height (cm)">
                            <Input v-model.number="form.config.default_height" type="number" step="0.1" min="0" />
                        </FormField>
                        <FormField label="Weight (kg)">
                            <Input v-model.number="form.config.default_weight" type="number" step="0.01" min="0" />
                        </FormField>
                    </div>
                </Card>
            </div>
                ...
                <Card mode="flat" title="Status">
                    <dl class="space-y-2.5 text-[13px]">
                        <div class="flex items-center justify-between">
                            ...
                            <dd class="font-mono text-gray-900 dark:text-white/95">{{ partner.code }}</dd>
                        </div>
                        <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <dd>
                                <Badge :variant="'neutral'" :solid="form.is_default" size="sm">
...
import { Card, Button, Badge, Input, Textarea, Switch, FormField, PasswordInput, Alert } from '@/Components/ui';
...
export default {
    ...
    components: {
        Head, SettingsShell,
        Card, Button, Badge, Input, Textarea, Switch, FormField, PasswordInput, Alert,
    ...
    data() {
        // Non-secret fields come back in full so the admin can see what is
        // configured and edit it in place. Secrets start blank and are only
        // fetched on an explicit reveal; an empty submit leaves them as-is.
        const initialCreds = {};
        Object.entries(this.schema || {}).forEach(([k, meta]) => {
            initialCreds[k] = meta.type === 'password'
                ? ''
                : (this.partner.credentials_public?.[k] ?? '');
        });
        ...
        return {
            ...
            form: useForm({
                ...
                mode: this.partner.mode || 'test',
                is_active: !!this.partner.is_active,
                is_default: !!this.partner.is_default,
                priority: this.partner.priority ?? 0,
                api_base_url: this.partner.api_base_url || '',
    ...
    methods: {
        submit() {
            ...
            this.form.put(route('admin.couriers.update', this.partner.id), {
                preserveScroll: true,
        ...
        async reveal(field) {
            ...
            try {
                const { data } = await window.axios.post(
                    route('admin.couriers.reveal', this.partner.id),
                    { field },
                );
        ...
        test() {
            this.testing = true;
            router.post(route('admin.couriers.test', this.partner.id), {}, {
                preserveScroll: true,
