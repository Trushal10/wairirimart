/**
 * Building a Swiper is not free: this theme stands up nine of them on the
 * homepage alone, and every one was constructed during initial parse — including
 * the carousels sitting several screens down. That work all lands inside the
 * window Lighthouse measures as blocking time.
 *
 * Carousels that start off-screen are built the first time they come near the
 * viewport instead. Anything already visible, and any browser without
 * IntersectionObserver, still builds immediately, exactly as before.
 *
 * Only used for carousels whose instance is not referenced after construction —
 * the ones with hover/destroy handlers keep building eagerly.
 */
function initSwiperWhenNear(selector, options) {
  ...
  if (typeof IntersectionObserver === "undefined") return build();

  var viewportHeight = window.innerHeight || document.documentElement.clientHeight;
  if (el.getBoundingClientRect().top < viewportHeight * 1.25) return build();
  ...
  var observer = new IntersectionObserver(
    ...
    // Generous margin: this only governs how early a deferred carousel wakes up
    // during scrolling, not what happens on load (the viewport test above owns
    // that), so it costs nothing up front and keeps a fast scroll from
    // outrunning the observer.
...
if ($(".tf-sw-effect").length > 0) {
  var swiper2 = initSwiperWhenNear(".tf-sw-effect", {
    spaceBetween: 0,
    speed: 2000,
    effect: "fade",
    observer: true,
...
if ($(".tf-sw-collection").length > 0) {
  ...
  var swiper = initSwiperWhenNear(".tf-sw-collection", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    speed: 1000,
    pagination: {
...
if ($(".tf-sw-categories").length > 0) {
  ...
  var swiper = initSwiperWhenNear(".tf-sw-categories", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    speed: 1000,
    pagination: {
...
if ($(".tf-sw-recent").length > 0) {
  ...
  var swiper = initSwiperWhenNear(".tf-sw-recent", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    pagination: {
...
if ($(".tf-sw-latest").length > 0) {
  ...
  var swiper = initSwiperWhenNear(".tf-sw-latest", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    observer: true,
    observeParents: true,
...
if ($(".tf-sw-products").length > 0) {
  ...
  var swiper = initSwiperWhenNear(".tf-sw-products", {
    speed: 1000,
    slidesPerView: mobile,
    spaceBetween: spacing,
    pagination: {
...
if ($(".tf-sw-products1").length > 0) {
  ...
  var swiper = initSwiperWhenNear(".tf-sw-products1", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    observer: true,
    observeParents: true,
...
if ($(".tf-sw-testimonial").length > 0) {
  ...
  var swiper = initSwiperWhenNear(".tf-sw-testimonial", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    speed: 1000,
    pagination: {
...
if ($(".tf-sw-shop-gallery").length > 0) {
  ...
  var swiper = initSwiperWhenNear(".tf-sw-shop-gallery", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    observer: true,
    observeParents: true,
...
if ($(".tf-sw-iconbox").length > 0) {
  ...
  var swiper = initSwiperWhenNear(".tf-sw-iconbox", {
    slidesPerView: mobile,
    spaceBetween: spacing,
    speed: 1000,
    pagination: {
...
if ($(".tf-product-header").length > 0) {
  var swiper = initSwiperWhenNear(".tf-product-header", {
    slidesPerView: 2,
    spaceBetween: 20,
    navigation: {
      clickable: true,
