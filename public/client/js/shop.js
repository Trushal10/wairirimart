...
(function ($) {
  ...
  var filterProducts = function () {
    ...
    function applyFilters() {
      ...
      // Prices render via number_format, so a four-figure price is "12,000.00"; parseFloat stops at the comma and returns 12. Strip everything that is not a digit or a dot instead of only the (never-present) dollar sign.
  ...
  var filterSort = function () {
    ...
    function applyFilter(sortValue, isListActive) {
      ...
      if (sortValue === "price-low-high") {
        products.sort(
          (a, b) =>
            parseFloat($(a).find(".current-price").text().replace(/[^\d.]/g, "")) -
            parseFloat($(b).find(".current-price").text().replace(/[^\d.]/g, ""))
        );
