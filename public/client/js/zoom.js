...
(function ($) {
  ...
  // Drift powers the desktop hover-to-zoom. It binds non-passive
  // touchstart/touchmove/touchend on the product image and calls
  // preventDefault() on all three, which on a phone blocks page
  // scrolling over the image and swallows the tap before it can become
  // a click — so the PhotoSwipe lightbox never opens. `handleTouch:false`
  // only disables Drift's own touch-zoom; it does NOT stop those
  // listeners. Phones don't need hover-zoom at all: tapping opens the
  // lightbox, which has pinch-zoom. So only wire Drift up on devices
  // that actually have a hovering, fine pointer.
  var has_hover_pointer = function () {
