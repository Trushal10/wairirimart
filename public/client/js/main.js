...
(function ($) {
  ...
  var variantPicker = function () {
    // Scope the active state to the picker-item so switching one option
    // (e.g. Weight) doesn't nuke the visual state of another (e.g. Colour).
    // Also skip when the pill is disabled (out-of-stock variants).
    function syncPillState($el) {
      if ($el.is('.disabled') || $el.attr('aria-disabled') === 'true') return;
      var $item = $el.closest('.variant-picker-item');
      $item.find('.variant-picker-label-value').text($el.data('value'));
      $item.find('.color-btn, .size-btn').removeClass('active');
      $el.addClass('active');
    }
    $(document).on('click', '.color-btn', function () { syncPillState($(this)); });
    $(document).on('click', '.size-btn', function () { syncPillState($(this)); });
  ...
  var clickModalSecond = function () {
    ...
    $(".btn-icon-action.wishlist").click(function (e) {
      e.preventDefault();
  ...
  var totalPriceVariant = function () {
    $(".tf-product-info-list,.tf-cart-item").each(function () {
      ...
      // Theme demo behaviour: pills carry their price in data-price. Real
      // variant pills don't — they resolve price from the variants payload in
      // variant-picker.js — so skip them here or this would overwrite the
      // resolved price with the page's initial one on every click.
      productItem.find(".color-btn, .size-btn").not(".tf-product-info-variant-groups .color-btn, .tf-product-info-variant-groups .size-btn").on("click", function () {
  ...
  /* Quantity +/- — product page, quick-add modal and cart rows all share it.
     The value is clamped to the input's min/max (max is the selected
     variant's stock, kept current by variant-picker.js) and a `change` event
     is fired so listeners such as the cart page's debounced update react to
     button clicks exactly as they do to typing. This is the only +/- handler:
     a second one on the same buttons stacks with it and double-steps. */
  function clampQuantity($input, value) {
    var min = parseInt($input.attr('min') || $input.data('min'), 10);
    var max = parseInt($input.attr('max') || $input.data('max'), 10);
  ...
  /* Paint the stepper to match what can actually be bought.
     A control the shopper cannot move must LOOK unmovable — an inert button
     that still reads as live invites a click that does nothing. So:
       cap < 2   hide the block outright; there is nothing to choose
       at 1      disable minus
       at cap    disable plus
       cap <= 5  show the low-stock note, which is also what explains the
                 disabled plus once the ceiling is reached
     Exposed because variant-picker.js re-runs it whenever the shopper switches
     variant and the cap moves with it.

     `clamp` is false while the shopper is still typing: rewriting the field on
     every keystroke fights them, because clearing it to type a new number
     momentarily parses as empty and would snap straight back to 1. The value is
     corrected once the field settles instead. */
  function syncQuantityUi(scope, clamp) {
    var $scope = scope ? $(scope) : $(document);
    $scope.find('.quantity-product').each(function () {
      var $input = $(this);
      var $block = $input.closest('[data-role="quantity-block"]');
      var $wrap  = $input.closest('.wg-quantity');
      var min = parseInt($input.attr('min') || $input.data('min'), 10);
      var max = parseInt($input.attr('max') || $input.data('max'), 10);
      if (!Number.isFinite(min) || min < 1) min = 1;

      var current = clampQuantity($input, parseInt($input.val(), 10));
      if (clamp !== false && String(current) !== String($input.val())) $input.val(current);

      // data-stock is the real availability; max is clamped to >= 1 so the
      // input stays usable, which is why it cannot answer "is there anything
      // to choose here" on its own.
      var stock = parseInt($block.attr('data-stock'), 10);
      if (!Number.isFinite(stock)) stock = Number.isFinite(max) ? max : min;

      if ($block.length) $block.prop('hidden', stock < 2);

      var atMin = current <= min;
      var atMax = Number.isFinite(max) && current >= max;
      $wrap.find('.btn-decrease')
        .toggleClass('is-disabled', atMin)
        .attr('aria-disabled', atMin ? 'true' : 'false')
        .attr('tabindex', atMin ? -1 : 0);
      $wrap.find('.btn-increase')
        .toggleClass('is-disabled', atMax)
        .attr('aria-disabled', atMax ? 'true' : 'false')
        .attr('tabindex', atMax ? -1 : 0);

      var $note = $block.find('[data-role="quantity-note"]');
      if ($note.length) {
        var low = stock > 1 && stock <= 5;
        $note.prop('hidden', !low);
        if (low) $note.text('Only ' + stock + ' left');
      }
    });
  }
  window.syncQuantityUi = syncQuantityUi;

  $(document).on('click', '.btn-increase, .btn-decrease', function (e) {
    e.preventDefault();
    var $btn = $(this);
    if ($btn.hasClass('is-disabled')) return;
    var $input = $btn.closest('.wg-quantity').find('.quantity-product');
    if (!$input.length) return;
    var current = parseInt($input.val(), 10);
    if (!Number.isFinite(current)) current = 1;
    var next = clampQuantity($input, current + ($btn.hasClass('btn-increase') ? 1 : -1));
    if (next === current) return;
    $input.val(next).trigger('change');
  });

  function quantityScope(el) {
    return $(el).closest('[data-role="quantity-block"], .tf-cart-item').first();
  }

  // Repaint the buttons as the shopper types, but leave the value alone.
  $(document).on('input', '.quantity-product', function () {
    syncQuantityUi(quantityScope(this), false);
  });

  // Typed values get the clamp once the field settles.
  $(document).on('change', '.quantity-product', function () {
    syncQuantityUi(quantityScope(this), true);
  });

  $(function () { syncQuantityUi(document, true); });
